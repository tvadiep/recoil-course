...
const eurSelector = selector({
    ...
    get: ({get}) => {
        let usd = get(usdAtom)
        const commissionEnable = get(commissionEnabledAtom)
        if (commissionEnable) {
    ...
    set: ({set, get}, newEurValue) => {
        ...
        if (get(commissionEnabledAtom)) {
            newUSDValue = addCommission(newUSDValue, get(commissionAtom))
        }
...
export const Selectors = () => {
    const [usdValue, setUsdValue] = useRecoilState(usdAtom)
    const [eurValue, setEurValue] = useRecoilState(eurSelector)
    return (
